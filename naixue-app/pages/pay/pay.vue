		...
		<modal :show="ensureAddressModalVisible" custom :mask-closable="false" :radius="0" width="90%">
			<view class="modal-content">
				...
				<view class="d-flex font-size-sm text-color-assist align-items-center justify-content-between mb-40">
					...
					<button type="primary" size="mini" plain class="change-address-btn" @tap="chooseAddress">
						修改地址
					</button>
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...mapMutations(['SET_ORDERCURRENT', 'SET_STORE', 'SET_ADDRESS']),
			//初始化函数
			init(id) {
				...
				return uniCloud
					...
					.then(res => {
						if(res.result.status === 0) {
							...
							.then(res => {
								console.log(res.result.orderInfo);
			...
			pay() {
				...
				uni.reLaunch({
					url: "/pages/take-foods/take-foods?order_id=" + '1606828864ced600c4'
				});
			}
		}
	}
